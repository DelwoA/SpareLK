    ...
    <div className="fixed right-20 bottom-16 z-50">
      <div className="size-16 group">
        <button
          ...
          className="size-16 rounded-full border border-gray-800 overflow-hidden shadow-lg shadow-black"
        >
          <img
            src="/chat.png"
            alt="chatbot"
            className="scale-150 hover:scale-[2] hover:rotate-12 hover:animate-pulse duration-100 ease-in-out"
          />
        </button>
        {!chatOpen && (
          <div className="w-40 rounded-lg text-center absolute bottom-[120%] right-0 shadow-lg shadow-black py-2 bg-main text-light origin-bottom-right scale-0 duration-75 group-hover:scale-100">
            Need Help?
          </div>
        )}
      </div>
      ...
      {/* chat */}
      <div
        className={`w-[500px] h-[700px] flex flex-col ${
          chatOpen ? "scale-100" : "scale-0"
        } duration-150 ease-in-out origin-bottom-right absolute border-2 border-gray-400 right-12 bottom-12 bg-white rounded-lg shadow-lg shadow-gray-400`}
      >
          ...
          {messages.map((msg, index) =>
            msg.sender == "user" ? (
              ...
              <div key={index} className={`flex justify-start`}>
                <div
                  className={`p-2 max-w-xs rounded-lg bg-light text-main border border-main`}
                >
                  {msg.text}
                  {/* {msg.text[0]}
                                    {
        ...
        <div className="p-3 flex items-center border-t border-gray-400">
          ...
          />
          <button
            className="ml-3 px-4 py-2 text-lg bg-blue-600 text-white rounded-lg"
            onClick={sendMessage}
          >
            Send
          </button>
      ...
      {chatOpen && (
        <div
          ...
          className={`w-[150vw] backdrop-blur-sm h-[150vh] bg-black/40 -z-10 fixed -right-20 -bottom-20`}
        ></div>
      )}
    </div>
  );
