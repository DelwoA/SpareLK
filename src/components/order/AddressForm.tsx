...
export default function AddressForm({
  ...
  function saveAddress() {
    const { cart, store, ...updatedUser } = user || {};
    api
      .put("user/" + user?._id, { ...updatedUser, address: address })
      .then((_response) => {
  ...
  return (
    <div
      className={`fixed ${
        isAddressFormOpened ? "flex" : "hidden"
      } items-center justify-center w-full h-full backdrop-blur-lg top-0 left-0`}
    >
      <div className="w-96 flex flex-col bg-white border border-gray-300 rounded-2xl p-6">
        <h3 className="self-center text-2xl font-semibold mb-4">
          Address Form
        </h3>
